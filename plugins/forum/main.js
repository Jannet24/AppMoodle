...
define(templates, function (filesTpl, discussionTpl, discussionsTpl, attachmentsTpl, workerCode) {
    var plugin = {
        ...
        routes: [
            ["forum/:courseId", "view_forum", "viewForum"],
            ["forum/discussions/:forumId", "show_discussion", "showDiscussion"],
            ["forum/discussion/:discussionId","show_discussion","showDiscuss"],
        ],
         ...
         *
         */
        viewForum: function(courseId) {
            // Loading ....
            //$("#info-" + cmid, "#panel-right").attr("src", "img/loadingblack.gif");

            // First, load the complete information of forums in this course.
            console.log("Entrando viewForum");
            var params = {
                "courseids[0]": courseId
            };
            

            MM.moodleWSCall("local_mobile_mod_forum_get_forums_by_courses",
                params,
                function(forums) {
               
                    var datos = {foros: forums,courses:MM.coursePlugins,courseId:courseId};
                    var html = MM.tpl.render(MM.plugins.forum.templates.view.html, datos);
                    MM.ocultarMenu();
         ...
         */
        showDiscussion: function(forumId) {
            var params = {
                "forumids[0]": forumId
            };
            ...
            MM.moodleWSCall("mod_forum_get_forum_discussions",
                params,
                // Success callback.
                function(data) {
                    /*
                ...
                var courseId = $(this).data("courseId");
                    var datos = {discussions: data,courses:MM.coursePlugins,courseId:courseId};
                    var html = MM.tpl.render(MM.plugins.forum.templates.discussions.html, datos);
                    $('#principal').html(html).trigger("create");
                },
                null,
                function (error) {
        ...
        showDiscuss:function(discussId){
            var params = {
                "discussionid": discussId,
                "sortdirection":"ASC"
            };
            MM.moodleWSCall("mod_forum_get_forum_discussion_posts",
                params,
                // Success callback.
                function(data) {
                //console.log(data);
                var courseId = $(this).data("courseId");
                var datos = {posts:data['posts'],courses:MM.coursePlugins,courseId:courseId};
                var html = MM.tpl.render(MM.plugins.forum.templates.discussion.html, datos);
                $('#principal').html(html).trigger("create");
            },null,
                function (error) {
                    console.log(error);

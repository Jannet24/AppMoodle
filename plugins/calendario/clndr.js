...
function defaults(tpl){
  var eventos = [];
  for (var i = 0; i < tpl.events.length; i++) {
    var fechaR =tpl.events[i].startdate;
    var len = tpl.events[i].startdate.length;
    var fecha =fechaR[len-4]+fechaR[len-3]+fechaR[len-2]+fechaR[len-1]+"/";

    switch (len) {
      case 10:
          fecha+= fechaR[3]+fechaR[4]+"/"+fechaR[0]+fechaR[1];
          break;
      case 8:
          fecha+= fechaR[2]+"/"+fechaR[0];
          break;
      case 9:
          if (fechaR[1]=="/") {
          fecha+= fechaR[2]+fechaR[3]+"/"+fechaR[0];
          }
          else
          {
            fecha+=fechaR[3]+"/"+fechaR[0]+fechaR[1];
          }
          break;
    }
    eventos.push({date: fecha,course: tpl.events[i].courseid, title: tpl.events[i].name, time: tpl.events[i].starttime, description: tpl.events[i].description})};
  console.log("Este Es MI TPL: "+tpl);
  console.log(tpl);
  $('#myCalendar').clndr({
    events: eventos,
    clickEvents: {
      click: function(target) {
        console.log(target);        
        MM.calendario1(target.date._i, target.events);
      }
  }
  });       

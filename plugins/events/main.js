...
var templates = [
    ...
    "root/externallib/text!root/plugins/events/calendario.html",
    
];

define(templates, function (eventsTpl, eventTpl, calendarTpl) {
    var plugin = {
        settings: {
            name: "events",
            type: "general",
        ...
        routes: [
            ...
            ["events/calendario", "show_calendar", "showCalendar"]
        ],
         ...
         */
         showCalendar:function(){
            ...
            MM.plugins.events.showEvents(150);
         },
        ...
        _getCalendarEventsSucces: function(response, days) {
            MM.log("Entro a _getCalendarEventsSucces");
            ...
            console.log("Este es el tpl");
            console.log(tpl);
            if(mostrarCalendario == false){
            ...
            {
                var html = MM.tpl.render(MM.plugins.events.templates.calendario.html);
                console.log("MIS EVENTOS:");
                console.log(tpl);
                $('#principal').html(html).trigger("create");
                $('#principal').css("display", "block");
                console.log("llego a defaults()");
                defaults(tpl);
                 console.log("Salio de defaults()");
        ...
        _getCalendarEventsFailure: function(m) {
             MM.log("Entro a _getCalendarEventsFailure");
            // Removing loading icon.
            $('a[href="' + MM.plugins.events.settings.menuURL + '"]', '#panel-left').removeClass('loading-row');
        ...
        showEvents: function(days) {
             MM.log("Entro a showEvents");
            //MM.panels.showLoading('principal');

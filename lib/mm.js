var MM = {
        ...
        MM.moodleWSCall('core_enrol_get_users_courses', {userid: site.get('userid')}, function(courses) {
           ...
           //var output = MM.tpl.render($('#calendario_template').html(), values);
            ...
            for (var el in courses){
                ...
                //Buscamos el maestro del curso
                var aCurso = {
                "courseid": storedCourse.courseid
                };
                storedCourseList.push(storedCourse);
                MM.moodleWSCall('core_enrol_get_enrolled_users', aCurso, function(users){
                    console.log("Respuesta de core_enrol_get_enrolled_users");
                    for (participante in users) {
                        if (users[participante].roles.length == 0) {
                            maestro.push(users[participante].id);
                            break;
                        }
                    }
                    var p =maestro.length -1;
                    storedCourseList[p].teacher = maestro[p];
                    var r = MM.db.insert('courses', storedCourseList[p]);
                    console.log(r);
                    console.log("Fin de Respuesta de core_enrol_get_enrolled_users");
                });
    ...
    login: function(username, password, siteurl){
       ...
       // alert(loginURL + " Intentando conectar " + username + password);
